...
from app.models.trip_model import TripPlan
...
# Call the weather API to get the weather data for the next 15 days
...
# Parse the JSON value from the AI or fetched from the DB into the required model
def parseJSONToPlanDetails(jsonData: Any, location) -> PlanDetails:
    # Convert parsed JSON into your response schema
    trip_plan = []
    for day in jsonData['trip_plan']:
        places = []
    ...
    plan = PlanDetails(
        id=str(uuid.uuid4()), 
        start_date=jsonData["best_start_date"], 
        title=f"""{location} Trip""", 
        days=trip_plan
    )

    return plan

# Call the API to create the trip plan
def generate_trip_plan(req: TripRequest) -> PlanDetails:
    db = SessionLocal()
    
    # Check Cache
    existing_plan = db.query(TripPlan).filter_by(location=req.location, days=req.days).first()
    if existing_plan:
        print("✅ Cache hit")
        plan = parseJSONToPlanDetails(existing_plan.response, req.location)
        return plan
    ...
    # Store data to DB for easier access in future
    new_plan = TripPlan(location=req.location, days=req.days, response=parsed)
    db.add(new_plan)
    db.commit()
    db.refresh(new_plan)
    db.close()

    trip_plan = parseJSONToPlanDetails(parsed, req.location)

    return trip_plan

    # Dummy data used for testing reduces AI API wait times
